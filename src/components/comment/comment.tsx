...
import { useDeleteCommentMutation } from '@/queries/useInteract'
...
const Comment = ({ data, comment, type = 'course' }: Props) => {
  ...
  //   comment?.votesOnComments?.some(
  //     (vote) => vote?.userId === user?.id && vote?.isUpVote,
  //   ),
  // )
  ...
  const handleDeleteComment = async () => {
    try {
      const res = await deleteCommentMutation.mutateAsync(comment.id)
      ...
      <div className="mt-4">
        <div className="flex gap-2">
          <Link href={`/profile/${comment.userId}`} className="relative">
            <Avatar
              src={generateMediaLink(comment.user.avatarFileId ?? '')}
              size="sm"
              className={cn(
                'aspect-square',
